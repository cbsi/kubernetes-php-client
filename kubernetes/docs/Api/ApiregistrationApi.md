# CBSi\Kubernetes\ApiregistrationApi
...
$api_instance = new CBSi\Kubernetes\Api\ApiregistrationApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getAPIGroup();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ApiregistrationApi->getAPIGroup: ', $e->getMessage(), PHP_EOL;
}
?>
```
