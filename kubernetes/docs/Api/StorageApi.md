...
$api_instance = new CBSi\Kubernetes\Api\StorageApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getAPIGroup();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling StorageApi->getAPIGroup: ', $e->getMessage(), PHP_EOL;
}
?>
```
