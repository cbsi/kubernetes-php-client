...
$api_instance = new CBSi\Kubernetes\Api\AppsApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getAPIGroup();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AppsApi->getAPIGroup: ', $e->getMessage(), PHP_EOL;
}
?>
```
