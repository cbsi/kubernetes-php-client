...
$api_instance = new CBSi\Kubernetes\Api\BatchApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getAPIGroup();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling BatchApi->getAPIGroup: ', $e->getMessage(), PHP_EOL;
}
?>
```
