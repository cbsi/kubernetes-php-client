# CBSi\Kubernetes\CertificatesApi
...
$api_instance = new CBSi\Kubernetes\Api\CertificatesApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getAPIGroup();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling CertificatesApi->getAPIGroup: ', $e->getMessage(), PHP_EOL;
}
?>
```
