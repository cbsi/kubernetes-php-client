# CBSi\Kubernetes\AuthorizationApi
...
$api_instance = new CBSi\Kubernetes\Api\AuthorizationApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getAPIGroup();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AuthorizationApi->getAPIGroup: ', $e->getMessage(), PHP_EOL;
}
?>
```
