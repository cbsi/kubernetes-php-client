# CBSi\Kubernetes\AuthenticationApi
...
$api_instance = new CBSi\Kubernetes\Api\AuthenticationApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getAPIGroup();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AuthenticationApi->getAPIGroup: ', $e->getMessage(), PHP_EOL;
}
?>
```
